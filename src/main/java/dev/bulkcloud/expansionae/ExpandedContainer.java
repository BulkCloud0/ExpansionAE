 ...
 */
public class ExpandedContainer extends UpgradeableContainer {

    public static final ContainerType<ExpandedContainer> TYPE = ContainerTypeBuilder
            .create(ExpandedContainer::new, ExpandedHost.class)
            .requirePermission(SecurityPermissions.BUILD)
            .build("expansionae_interface");
    ...
    public ExpandedContainer(int id, final PlayerInventory ip, final ExpandedHost te) {
        super(TYPE, id, ip, te.getInterfaceDuality().getHost());

        ExpandedDuality duality = te.getInterfaceDuality();
        storageSlots = duality.getStorage().getSlots();

        for (int x = 0; x < duality.getPatterns().getSlots(); x++) {
        ...
        for (int x = 0; x < duality.getConfig().getSlots(); x++) {
            this.addSlot(new FakeSlot(duality.getConfig(), x), SlotSemantic.CONFIG);
        }

        for (int x = 0; x < duality.getStorage().getSlots(); x++) {
