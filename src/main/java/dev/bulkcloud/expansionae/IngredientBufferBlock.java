...
public final class IngredientBufferBlock extends AEBaseTileBlock<IngredientBufferTile> {
    public IngredientBufferBlock() { super(defaultProps(Material.IRON)); }
    ...
    public ActionResultType onActivated(World world, BlockPos pos, PlayerEntity player, Hand hand,
        ...
        if (FluidUtil.interactWithFluidHandler(player, hand, tile.getFluidInventory())) {
            return ActionResultType.func_233537_a_(world.isRemote());
        }
        if (!world.isRemote()) {
            ContainerOpener.openContainer(IngredientBufferContainer.TYPE, player,
