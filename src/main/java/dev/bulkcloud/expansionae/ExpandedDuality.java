...
// Adapted for ExpansionAE on 2026-09-18: independently sized inventories.
...
public class ExpandedDuality extends appeng.helpers.DualityInterface implements IGridTickable, IStorageMonitorable, IInventoryDestination, IAEAppEngInventory,
    ...
    private boolean updateStorage() {
        ...
        for (int x = 0; x < this.storage.getSlots(); x++) {
    ...
    public boolean pushPattern(final ICraftingPatternDetails patternDetails, final CraftingInventory table) {
        ...
        for (final Direction s : possibleDirections) {
            ...
            if (te instanceof IInterfaceHost) {
                try {
                    if (this.sameNetwork((IInterfaceHost) te)) {
                        continue;
                    }
                } catch (final GridAccessException e) {
                    continue;
    ...
    private boolean sameNetwork(IInterfaceHost host) throws GridAccessException {
        if (!(host instanceof IActionHost)) return false;
        IGridNode node = ((IActionHost) host).getActionableNode();
        return node != null && node.getGrid() == this.gridProxy.getGrid();
    }
    ...
    public ITextComponent getTermName() {
        ...
        for (final Direction direction : possibleDirections) {
            ...
            if (directedTile instanceof IInterfaceHost) {
                try {
                    if (this.sameNetwork((IInterfaceHost) directedTile)) {
                        continue;
                    }
                } catch (final GridAccessException e) {
                    continue;
    ...
    private class InterfaceRequestContext implements Comparable<Integer> {
        ...
        public int compareTo(Integer o) {
            return Integer.compare(ExpandedDuality.this.priority, o);
        }
    }

    private class InterfaceInventory extends MEMonitorIInventory {

        public InterfaceInventory(final ExpandedDuality tileInterface) {
            super(new AdaptorItemHandler(tileInterface.storage));
            this.setActionSource(new MachineSource(ExpandedDuality.this.iHost));
        }
        ...
        public IAEItemStack extractItems(final IAEItemStack request, final Actionable type, final IActionSource src) {
            final Optional<InterfaceRequestContext> context = src.context(InterfaceRequestContext.class);
            final boolean hasLowerOrEqualPriority = context.map(c -> c.compareTo(ExpandedDuality.this.priority) <= 0)
                    .orElse(false);
    ...
    private class Accessor implements IStorageMonitorableAccessor {
        ...
        public IStorageMonitorable getInventory(IActionSource src) {
            return ExpandedDuality.this.getMonitorable(src, ExpandedDuality.this);
        }
