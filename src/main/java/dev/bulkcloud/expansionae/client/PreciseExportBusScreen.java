...
public final class PreciseExportBusScreen extends UpgradeableScreen<PreciseExportBusContainer> {
    ...
    public PreciseExportBusScreen(PreciseExportBusContainer container, PlayerInventory player,
    ...
    protected void init() {
        super.init();
        batchMode = addButton(new Button(guiLeft + 108, guiTop + 64, 80, 18,
                modeText(), b -> container.toggleBatchMode()));
    }

    private ITextComponent modeText() {
        return new TranslationTextComponent(container.isMultipleMode()
                ? "gui.expansionae.precise.multiple"
                : "gui.expansionae.precise.exact");
    }
    ...
    protected void updateBeforeRender() {
        ...
        if (batchMode != null) batchMode.setMessage(modeText());
    }
}
