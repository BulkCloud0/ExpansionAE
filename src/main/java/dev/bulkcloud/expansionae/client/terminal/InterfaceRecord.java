...
public class InterfaceRecord implements Comparable<InterfaceRecord> {
     ...
     * {@link appeng.container.implementations.ExpandedTerminalContainer}
     */
    ...
    public InterfaceRecord(long serverId, int slots, long order, ITextComponent name) {
        ...
        this.searchName = this.displayName.toLowerCase(java.util.Locale.ROOT);
    ...
    public int compareTo(@Nonnull final InterfaceRecord o) {
        int orderComparison = Long.compare(this.order, o.order);
        return orderComparison != 0 ? orderComparison : Long.compare(this.serverId, o.serverId);
    }
