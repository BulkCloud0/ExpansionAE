...
public class ExpandedTerminalScreen extends AEBaseScreen<ExpandedTerminalContainer> {
    ...
    public void postInventoryUpdate(boolean clearExistingData, long inventoryId, CompoundNBT invData) {
        if (clearExistingData) {
            ...
            final InterfaceRecord current = this.getById(inventoryId, invData.getLong("sortBy"), un, invData.getInt("slots"));
    ...
    private void refreshList() {
        ...
        final String searchFilterLowerCase = (this.searchField == null ? "" : this.searchField.getText()).toLowerCase(java.util.Locale.ROOT);
    ...
    private InterfaceRecord getById(final long id, final long sortBy, final ITextComponent name, int slots) {
        InterfaceRecord o = this.byId.get(id);

        if (o == null) {
            this.byId.put(id, o = new InterfaceRecord(id, MathHelper.clamp(slots, 1, 9), sortBy, name));
            this.refreshList = true;
        }
