...
public final class TagExportBusScreen extends UpgradeableScreen<TagExportBusContainer> {
    private final SettingToggleButton<RedstoneMode> redstone;
    private AETextField whitelist;
    private AETextField blacklist;

    public TagExportBusScreen(TagExportBusContainer container, PlayerInventory player,
    ...
    protected void updateBeforeRender() {
        ...
        if (whitelist != null && !whitelist.isFocused()
                && !whitelist.getText().equals(container.whitelist)) {
            whitelist.setText(container.whitelist == null ? "" : container.whitelist);
        }
        if (blacklist != null && !blacklist.isFocused()
                && !blacklist.getText().equals(container.blacklist)) {
            blacklist.setText(container.blacklist == null ? "" : container.blacklist);
        }
    }
}
