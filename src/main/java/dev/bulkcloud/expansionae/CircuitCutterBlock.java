...
public final class CircuitCutterBlock extends AEBaseTileBlock<CircuitCutterTile> {
    public CircuitCutterBlock() {
    ...
    public ActionResultType onActivated(World world, BlockPos pos, PlayerEntity player, Hand hand,
        ...
        CircuitCutterTile tile = getTileEntity(world, pos);
        if (tile == null) return ActionResultType.PASS;

        if (FluidUtil.interactWithFluidHandler(player, hand, tile.getFluidHandler())) {
            return ActionResultType.func_233537_a_(world.isRemote());
        }

        if (!world.isRemote()) {
            ContainerOpener.openContainer(CircuitCutterContainer.TYPE, player,
