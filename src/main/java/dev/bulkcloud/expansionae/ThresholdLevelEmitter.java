...
// Adapted from AE2 8.4 LevelEmitterPart for ExtendedAE threshold hysteresis.
...
public final class ThresholdLevelEmitter extends UpgradeablePart implements IEnergyWatcherHost, IStackWatcherHost,
    ...
    private long upperValue = 0;
    private long lowerValue = 0;
    private boolean thresholdState = false;
    ...
    public ThresholdLevelEmitter(final ItemStack is) {
        super(is);
    ...
    public boolean isLevelEmitterOn() {
        ...
        if (!this.getProxy().isActive() || this.lowerValue > this.upperValue) {
            return false;
        }
        ...
        final boolean invert = this.getConfigManager()
                .getSetting(Settings.REDSTONE_EMITTER) == RedstoneMode.LOW_SIGNAL;

        // Schmitt-trigger style hysteresis: while currently on, the lower
        // threshold is used to decide when to switch off. While off, the upper
        // threshold is used to decide when to switch on.
        final boolean logicalState = invert != this.thresholdState;
        final boolean active = logicalState
                ? this.lastReportedValue >= this.lowerValue
                : this.lastReportedValue >= this.upperValue;
        final boolean newState = invert != active;
        if (newState != this.thresholdState) {
            this.thresholdState = newState;
    ...
    public void setUpperValue(long value) {
        this.upperValue = Math.max(0, value);
        this.reportingValue = this.upperValue;
        this.configureWatchers();
        this.getHost().markForSave();
    }

    public void setLowerValue(long value) {
        this.lowerValue = Math.max(0, value);
        this.configureWatchers();
        this.getHost().markForSave();
    }
    ...
    public boolean onPartActivate(final PlayerEntity player, final Hand hand, final Vector3d pos) {
        if (!isRemote()) {
            ContainerOpener.openContainer(ThresholdLevelEmitterContainer.TYPE, player, ContainerLocator.forPart(this));
        }
        return true;
    }
    ...
    public void readFromNBT(final CompoundNBT data) {
        ...
        this.thresholdState = data.getBoolean("thresholdState");
        this.reportingValue = this.upperValue;
    ...
    public void writeToNBT(final CompoundNBT data) {
        ...
        data.putBoolean("thresholdState", this.thresholdState);
