...
- unsafe nesting of custom ICellHandler storage whose backing semantics are unknown;
- regressions in the bidirectional nesting veto between ExpansionAE DISKs and native AE2 cells;
- packaged-JAR release metadata drift (single distributable JAR, mod version, dependency ranges and manifest version);
...
A successful JAR build does not replace these runtime gates. The required baseline is green build/JUnit + packaged-JAR sanity + dedicated-server write/read restart smoke + client Xvfb model/tooltip smoke on Forge 36.2.42 + AE2 8.4.7 + MCP `20210309-1.16.5`. The package sanity gate requires exactly one distributable JAR, checks the filename/manifest/mods.toml against `mod_version`, verifies Minecraft/Forge/AE2 dependency ranges from `gradle.properties`, and requires the ExpansionAE Mixin config/class in the JAR.
...
ExpansionAE DISKs are never nestable inside another ExpansionAE DISK, even when currently empty. Because contents live in a UUID-backed external store, an empty DISK can gain contents later through another alias after the nesting decision was made; forbidding all ExpansionAE→ExpansionAE nesting closes that time-of-check/time-of-use capacity bypass. Healthy empty native AE2 item Storage Cells remain nestable inside an ExpansionAE DISK, matching AE2's normal behavior for empty cells. Other cell-registry items that do not implement `IStorageCell` are treated as opaque custom storage and rejected by default: an apparently empty custom cell may use an external alias-backed store and gain contents after nesting. AE2's Creative Storage Cell is the runtime test case for this rule. Special `IStorageCell` implementations that explicitly return `storableInStorageCell()==true` continue to follow AE2's native opt-in semantics.
